...
public class DefaultMOTable<R extends MOTableRow, C extends MOColumn, M extends MOTableModel<R>>
  ...
  public void commit(SubRequest request) {
    ...
    OID colIndex = getIndexPart(cellOID);
    OID fullIndex = null;
    Integer colId = null;
    Integer rowId = null;
    if ((cellOID.size() >= oid.size() + 2) && (cellOID.startsWith(oid))) {
      colId = cellOID.get(oid.size() + 1);
      rowId = cellOID.get(oid.size());
      fullIndex = new OID(new int[] {rowId, colId});
    }
    ...
    synchronized (model) {
      ...
      if (hasNewRows(request.getRequest())) {
        ...
        if (!model.containsRow(row.getIndex())) {
    	  ...
    	  if(rowId == null)
        	  row = model.getRow(fullIndex);
          else{
        	  OID rOID = new OID(rowId.toString());
        	  row = model.getRow(rOID);
          }
      }
